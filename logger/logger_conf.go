...
type Config struct {
	LevelCh    chan enum.LoggerLevel
	TimeFormat string `yaml:"timeFormat"`

	// 完整的文件路径名
	Filepath        string `yaml:"filepath"`
	Filename        string `yaml:"filename"`
	FilenameSuffix  string `yaml:"fileSuffix"`
...
func (c *Config) SetLevel(level enum.LoggerLevel) {
	c.LevelCh <- level
}
...
func (c *Config) makeFilename() string {
	...
	return fmt.Sprintf("%s/%s%s.%s", c.Filepath, c.Filename, c.FilenameSuffix, c.FileExtension)
}
...
func NewConfig(opts ...Option) Config {
	conf := Config{
		...
		FileMaxAge:     6 * 30,
		FileCompress:   true,
		LevelCh:        make(chan enum.LoggerLevel, 1),
	}
...
// WithLevel 日志级别
func WithLevel(level enum.LoggerLevel) Option {
